...
curl --noproxy '*' -s -o /dev/null -w "app.localhost:$HTTP_PORT /            -> %{http_code}\n" -H 'Host: app.localhost' "http://127.0.0.1:$HTTP_PORT/"
curl --noproxy '*' -s -o /dev/null -w "app.localhost:$HTTP_PORT /api/health  -> %{http_code}\n" -H 'Host: app.localhost' "http://127.0.0.1:$HTTP_PORT/api/health"
