...
def skill_path(name: str) -> str | None:
    ...
    for base in (Cfg.DATA / "components" / name, Cfg.COMPONENTS_DIR / name):
        p = base / "SKILL.md"
        if p.is_file():
            return str(p)
    return None
...
def load_prompt(prompt_name: str) -> tuple[str, str]:
    """兼容执行器接口；Prompt 身份和版本由 Catalog 管理。"""
    definition = catalog.resolve(prompt_name)
    return definition.content, definition.version
...
def save_prompt_override(prompt_name: str, version: str, content: str) -> dict:
    item = catalog.save_override(prompt_name, version, content)
    return {"ok": True, **item.metadata()}


def delete_prompt_override(prompt_name: str) -> dict:
    """删除覆盖，回落内置模板"""
    catalog.delete_override(prompt_name)
    return {"ok": True}
