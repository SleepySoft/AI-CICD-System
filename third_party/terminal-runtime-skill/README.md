...
## 分层架构：为什么是 ATR + tmux 而不是直接用 tmux

一个自然的问题是：tmux 本身就能让会话持久化（`send-keys` / `capture-pane`），agent 直接用 tmux 不行吗？

可以，但 tmux 和 ATR 解决的是两个不同层的问题：

```text
tmux 解决“会话活着”（持久层）：进程不随服务/终端死亡，可随时 attach
ATR  解决“agent 看得懂屏幕、动得对键盘”（观察/控制层）
```

agent 裸用 tmux 时需要自己解决：

- **输入编码**：TUI 会切换终端模式（application cursor keys、bracketed paste），同一个方向键在不同模式下要发不同字节序列，`send-keys` 不会替你做这个判断。
- **状态判断**：`capture-pane` 只是快照，没有“屏幕稳定”“输入就绪”“更新序号”的概念，agent 得自己轮询 diff，容易在程序刷新中途下手。
- **决策信号**：没有光标位置、模式状态、`confirmation_likely` / `error_likely` 这类可直接决策的结构化字段。
- **协作与安全**：没有 session lock、human takeover、危险命令拦截、audit log；人和 agent 同时操作会互相打架。
- **协议成本**：每个 agent 会话都要从通用知识里现场推导 tmux 的正确用法（引号转义、目标指定、literal 模式），高频出错点。

因此本项目的架构是：**tmux 在下面保证会话不死，ATR 在上面提供稳定的 HTTP 原语**。agent 不需要感知 tmux 的存在；人类则随时可用 `tmux attach -t atr-<session-id>` 直接上手，两条路并存。
...
deploy/
  terminal-runtime.service             # systemd unit（生产部署）
  README.md                            # 生产部署指南
references/
  dialog.md                            # 工具用法与示例
tests/
  test_terminal_runtime.py             # 平台无关的单元测试
  e2e_wsl.sh                           # tmux 后端持久化端到端验证（Linux/WSL 运行）
...
# 生产/持久化相关
export ATR_DEFAULT_BACKEND=tmux        # pty | tmux，tmux 会话可跨重启存活
export ATR_TMUX_PREFIX=atr-            # 自动找回时识别本会话的 tmux 前缀
export ATR_TMUX_ADOPT=1                # 启动时自动 adopt 存量 tmux 会话
export ATR_ALLOW_INSECURE_LISTEN=0     # 非 loopback 且无 token 时拒绝启动
python scripts/terminal_runtime_service.py
```
...
## 15.5 生产部署与会话持久（tmux 后端）

默认 `pty` 后端下，会话进程是 service 的子进程：**service 重启 = 会话全灭**。
生产环境请使用 `tmux` 后端：目标程序跑在独立的 tmux server 中，service 只是
attach 进去的观察者，崩溃/重启/升级均不影响会话；service 启动时会自动
adopt 所有存量会话，agent 用原来的 session id 即可找回。

```bash
# 服务端
export ATR_DEFAULT_BACKEND=tmux
# agent 侧创建（幂等）
python scripts/terminal_runtime_client.py create \
  --session-id kimi-main --command kimi --backend tmux --ensure
# service 重启后手动触发找回（通常不需要，启动时已自动 adopt）
python scripts/terminal_runtime_client.py reattach
```

完整的 systemd 部署步骤、安全红线和升级流程见 [deploy/README.md](deploy/README.md)。

安全闸门：绑定非 loopback 地址且未设置 `ATR_API_TOKEN` 时，服务拒绝启动。
