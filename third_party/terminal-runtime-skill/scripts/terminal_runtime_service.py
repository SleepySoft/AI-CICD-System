...
# Backend used when a session request does not specify one.
#   "pty"  : the service spawns the process directly (sessions die with the service)
#   "tmux" : the process lives in a tmux server and survives service restarts
DEFAULT_BACKEND = os.environ.get("ATR_DEFAULT_BACKEND", "pty")
TMUX_BIN = os.environ.get("ATR_TMUX_BIN", "tmux")
TMUX_PREFIX = os.environ.get("ATR_TMUX_PREFIX", "atr-")
# On startup, adopt tmux sessions left over from a previous service run so that
# agents can simply re-discover them via GET /sessions.
AUTO_ADOPT_TMUX = os.environ.get("ATR_TMUX_ADOPT", "1") != "0"
# Safety gate: refuse to listen on a non-loopback address without an API token
# unless explicitly overridden.
ALLOW_INSECURE_LISTEN = os.environ.get("ATR_ALLOW_INSECURE_LISTEN", "") == "1"
...
class CreateSessionRequest(BaseModel):
    ...
    # "pty" spawns the process directly; "tmux" runs it inside a tmux server so
    # it survives service restarts and can be re-attached. None = service default.
    backend: Optional[Literal["pty", "tmux"]] = None
    # If True and a session with this id already exists, return it instead of
    # failing. Makes session creation idempotent so agents can safely "ensure".
    ensure: bool = False
...
# =============================================================================
# tmux persistence backend helpers
# =============================================================================

def tmux_session_name(session_id: str) -> str:
    """Map an ATR session id to a tmux session name.

    tmux target syntax treats '.' and ':' specially, so they (and anything
    else unusual) are folded to '-'. The ATR prefix lets the service find
    its own sessions again after a restart.
    """
    safe = re.sub(r"[^A-Za-z0-9_-]", "-", session_id).strip("-") or "session"
    return (TMUX_PREFIX + safe)[:60]


def tmux_available() -> bool:
    try:
        proc = subprocess.run([TMUX_BIN, "-V"], capture_output=True, timeout=5)
...
def tmux_command_string(command: Union[str, List[str]], shell: bool) -> str:
    """Render the requested command as a single shell-command string for tmux."""
    if isinstance(command, list):
        return " ".join(shlex.quote(x) for x in command) if not shell else " ".join(command)
    return command


def build_tmux_client_argv(
    name: str,
    create: bool,
    ...
    """Argv for the tmux *client* that the service spawns inside its own PTY.

    create=True  -> start a new detached-able session running `command` and attach
    create=False -> attach to an already-running session (reattach after restart)
    """
    if create:
        if command is None:
            raise ValueError("tmux create requires a command")
        argv = [TMUX_BIN, "-2", "new-session", "-s", name, "-x", str(cols), "-y", str(rows)]
        if cwd:
            argv += ["-c", cwd]
        for k, v in (env or {}).items():
            argv += ["-e", f"{k}={v}"]
        argv += ["--", tmux_command_string(command, shell)]
        return argv
    return [TMUX_BIN, "-2", "attach-session", "-t", name]


def parse_tmux_list(output: str, prefix: str) -> List[Dict[str, Any]]:
    """Parse `tmux list-sessions -F ...` output into adoption candidates."""
    sessions: List[Dict[str, Any]] = []
    for line in output.splitlines():
        parts = line.split("\t")
        if len(parts) < 4:
            continue
        name, pane_command, pane_path, created = parts[0], parts[1], parts[2], parts[3]
        ...
        sessions.append({
            ...
            "command": pane_command or "unknown",
            "cwd": pane_path or None,
            "created_at": created_at,
        })
    return sessions


def tmux_list_sessions(prefix: str = TMUX_PREFIX) -> List[Dict[str, Any]]:
    fmt = "#{session_name}\t#{pane_current_command}\t#{pane_current_path}\t#{session_created}"
    try:
        proc = subprocess.run(
            [TMUX_BIN, "list-sessions", "-F", fmt],
            capture_output=True,
            timeout=5,
            text=True,
        )
    ...
    if proc.returncode != 0:
        return []
    return parse_tmux_list(proc.stdout, prefix)


def tmux_kill_session(name: str) -> None:
    try:
        subprocess.run([TMUX_BIN, "kill-session", "-t", name], capture_output=True, timeout=5)
    except Exception:
        pass


def insecure_listen_reason(host: str, api_token: str, allow_insecure: bool) -> Optional[str]:
    """Return a human-readable reason if the listen config is unsafe, else None."""
    if allow_insecure or api_token:
        return None
    if host in ("127.0.0.1", "localhost", "::1"):
        return None
    return (
        f"refusing to listen on non-loopback address {host!r} without ATR_API_TOKEN; "
        "set ATR_API_TOKEN, or set ATR_ALLOW_INSECURE_LISTEN=1 to override"
    )
...
class TerminalSession:
    def __init__(self, req: CreateSessionRequest, attach_existing: bool = False):
        ...
        self.backend = req.backend or DEFAULT_BACKEND
        # attach_existing=True means the tmux session is already running (e.g.
        # adopted after a service restart) and must not be re-created.
        self.attach_existing = attach_existing
        self.tmux_name = tmux_session_name(self.id) if self.backend == "tmux" else None
        self._tmux_alive: Optional[bool] = None
        self._tmux_alive_checked_at = 0.0
    ...
    # Lifecycle
    # ---------------------------------------------------------------------
    def _spawn_argv(self) -> tuple[Union[str, List[str]], bool, Optional[str]]:
        """Resolve (argv, shell, cwd) for the process placed inside our PTY.

        For the tmux backend this is the tmux *client*; the real target
        program lives in the tmux server and outlives this service.
        """
        if self.backend != "tmux":
            return normalize_command(self.command, self.shell), self.shell, self.cwd
        if not tmux_available():
            raise RuntimeError("tmux backend requested but tmux is not available")
        if self.attach_existing or tmux_has_session(self.tmux_name or ""):
            return build_tmux_client_argv(self.tmux_name or "", create=False), False, None
        return (
            build_tmux_client_argv(
                self.tmux_name or "",
                create=True,
    ...
    def start(self) -> None:
        ...
        argv, shell, cwd = self._spawn_argv()
        ...
        if self.backend == "tmux":
            # Never nest tmux clients; the target program's env is passed to
            # `tmux new-session -e` instead of inherited from the service.
            env.pop("TMUX", None)
        else:
            env.update(self.env)
    ...
    def stop(self, actor: str = "system", destroy: bool = True) -> None:
        """Terminate the session.

        destroy=True kills the underlying program (for tmux: the whole tmux
        session). destroy=False only detaches the service-side client, leaving
        a tmux session alive so it can be re-adopted later.
        """
        with self.lock:
            self.state = SessionState.STOPPING
        self._audit(actor, "session_stopping", {"destroy": destroy})
        if self.process and self.process.poll() is None:
            ...
            try:
                ...
                try:
                    ...
                    self.process.kill()
        if destroy and self.backend == "tmux" and self.tmux_name:
            tmux_kill_session(self.tmux_name)
        ...
        self._emit("session_stopped", {"destroy": destroy})
        self._audit(actor, "session_stopped", {"destroy": destroy})

    def detach(self, actor: str = "system") -> None:
        """Detach the service from the session without killing the program.

        Only meaningful for the tmux backend; pty sessions are stopped.
        """
        if self.backend == "tmux":
            self.stop(actor=actor, destroy=False)
        else:
            self.stop(actor=actor, destroy=True)
    ...
    def _reader_loop(self) -> None:
        try:
            ...
            # Never let the reader die silently: a dead reader leaves the
            # session looking RUNNING while the screen freezes forever.
            with self.lock:
                if self.state in (SessionState.RUNNING, SessionState.STARTING):
                    self.state = SessionState.FAILED
            self._emit("reader_error", {"error": repr(e)})
            self._audit("system", "reader_error", {"error": repr(e)})
    ...
    # Process health
    # ---------------------------------------------------------------------
    def _tmux_session_alive(self) -> Optional[bool]:
        """Throttled check that the backing tmux session still exists.

        Returns None for non-tmux backends. The tmux client normally exits
        when its session dies, so this is a belt-and-braces signal that also
        covers the case where the client was killed from outside.
        """
        if self.backend != "tmux" or not self.tmux_name:
            return None
        if now() - self._tmux_alive_checked_at < 5.0:
            return self._tmux_alive
        self._tmux_alive = tmux_has_session(self.tmux_name)
        self._tmux_alive_checked_at = now()
        return self._tmux_alive
        ...
        with self.lock:
            ...
            obs = {
                ...
                "process": {
                    ...
                    "tmux_alive": self._tmux_session_alive(),
                },
                "screen": {
                    "rows": self.rows,
                    "cols": self.cols,
...
class TerminalRuntime:
    ...
    def adopt_tmux_sessions(self) -> List[str]:
        """Re-attach to tmux sessions left behind by a previous service run.

        This is what makes sessions recoverable: after a service restart the
        agent simply lists sessions and finds the same ids again.
        """
        adopted: List[str] = []
        for info in tmux_list_sessions():
            sid = info["session_id"]
            with self.lock:
                if sid in self.sessions or len(self.sessions) >= DEFAULT_MAX_SESSIONS:
                    continue
            req = CreateSessionRequest(
                ...
                backend="tmux",
                owner="adopted",
                purpose="adopted after service restart",
                tags=["adopted"],
            )
            try:
                s = TerminalSession(req, attach_existing=True)
                s.start()
            except Exception:
                # Session may have died between listing and attach; skip it.
                continue
            with self.lock:
                self.sessions[sid] = s
            s._audit("system", "session_adopted", {"tmux_session": info["tmux_name"]})  # noqa: SLF001
            adopted.append(sid)
        return adopted

    def shutdown(self) -> None:
        """Service shutdown: detach from tmux sessions (they keep running),
        stop pty sessions (their processes are our children)."""
        with self.lock:
            sessions = list(self.sessions.values())
        for s in sessions:
            try:
                if s.state in (SessionState.RUNNING, SessionState.STARTING):
                    s.detach(actor="shutdown")
            except Exception:
                pass
...
async def lifespan(app: FastAPI):
    reason = insecure_listen_reason(HOST, API_TOKEN, ALLOW_INSECURE_LISTEN)
    if reason:
        raise RuntimeError(reason)
    if AUTO_ADOPT_TMUX and tmux_available():
        adopted = runtime.adopt_tmux_sessions()
        if adopted:
            print(f"[atr] adopted {len(adopted)} tmux session(s): {', '.join(adopted)}", flush=True)
    yield
    # Detach from tmux sessions (they keep running); stop pty sessions.
    runtime.shutdown()


app = FastAPI(title="Agent Terminal Runtime", version="1.1.0", lifespan=lifespan)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def health() -> Dict[str, Any]:
    return {
        ...
        "uptime_sec": max(0, int(now() - START_TIME)),
...
def create_session(req: CreateSessionRequest, authorization: Optional[str] = Header(default=None)) -> Dict[str, Any]:
    ...
    try:
        ...
        if not (req.ensure and req.id and "already exists" in str(e)):
            raise HTTPException(status_code=400, detail=str(e))
        s = runtime.get(req.id)
        created = False
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
    time.sleep(0.1)
    return {"ok": True, "session_id": s.id, "created": created, "observation": s.observe("agent")}


@app.post("/sessions/reattach")
def reattach_sessions(authorization: Optional[str] = Header(default=None)) -> Dict[str, Any]:
    """Adopt any tmux sessions that are alive but not currently registered.

    Safe to call at any time; agents can use it to re-discover sessions after
    a service restart.
    """
    require_auth(authorization)
    if not tmux_available():
        raise HTTPException(status_code=400, detail="tmux is not available on this host")
    adopted = runtime.adopt_tmux_sessions()
    return {"ok": True, "adopted": adopted, "sessions": [s["session_id"] for s in runtime.list()]}
...
if __name__ == "__main__":
    reason = insecure_listen_reason(HOST, API_TOKEN, ALLOW_INSECURE_LISTEN)
    if reason:
        raise SystemExit(f"[atr] {reason}")
    if DEFAULT_BACKEND == "tmux" and not tmux_available():
        print("[atr] warning: ATR_DEFAULT_BACKEND=tmux but tmux is not installed")
    import uvicorn
