...
def cmd_create(args: argparse.Namespace) -> None:
    ...
    if args.shell:
        body["shell"] = True
    print_json(request("POST", "/sessions", body))


def cmd_reattach(args: argparse.Namespace) -> None:
    print_json(request("POST", "/sessions/reattach"))
...
def main() -> None:
    ...
    sp = sub.add_parser("create", help="create a session (use --backend tmux for persistence)")
    sp.add_argument("--session-id", default=DEFAULT_SESSION_ID)
    sp.add_argument("--command", required=True, help="program to run, e.g. 'kimi' or 'vim file.txt'")
    sp.add_argument("--backend", choices=["pty", "tmux"], default=None, help="default: service setting (ATR_DEFAULT_BACKEND)")
    sp.add_argument("--ensure", action="store_true", help="idempotent: reuse the session if the id already exists")
    ...
    sp = sub.add_parser("reattach", help="adopt orphaned tmux sessions (e.g. after a service restart)")
    sp.set_defaults(func=cmd_reattach)
