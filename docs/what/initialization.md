...
重新进入初始化（经本机恢复子命令或初始化状态重置重开向导后）的语义是**恢复式收敛（recover）**：
保留并复用既有 `.env` 秘密、数据目录与已部署容器，以幂等检查-执行把现实收敛到声明状态；凭据漂移
以 `.env` 为准由组件 hook 对齐现实。向导不提供“全新初始化/一键清空”路径，彻底重建是显式运维操作
（备份 → 停栈 → 删数据 → 重置初始化状态）。可能令既有数据不可用的动作（如 `rotation_risk: critical`
的密钥轮换）必须列入计划的破坏性动作、默认不选中并二次确认。决策见
[ADR-0039](../adr/0039-reinit-recover-over-fresh.md)。
...
6. **计划确认**：以组件时间线展示 `skip/create/start/wait/configure/verify/restart`；警告和破坏性
  动作置顶。此阶段可主动使用唯一一次凭据显示与 JSON 下载机会：账号与密钥/令牌完整显示；口令仅在
  页面一次性完整显示（用户主动点击），不写入任何文件，需直接转存密码管理器；下载 JSON 只含账号
  与机器密钥类明文，不含口令。
...
不反向读取 `.env`；响应设置 `no-store`，服务端原子消耗机会，第二次调用必须拒绝。口令类秘密仅在
页面响应中返回一次用于完整显示，不进入下载文件；下载 JSON 仅含账号与机器密钥类明文。前端立即触发
本地 JSON 下载，显示内容不写入草稿、计划、日志、localStorage 或诊断数据。

秘密的防丢持久化采用主密钥 + 加密快照（[ADR-0041](../adr/0041-secret-snapshot-and-master-key.md)）：
初始化生成主密钥，系统唯一副本存于 `secrets/master.key`（chmod 600，永不入库/落 data），明文仅在
初始化完成时向操作者展示一次；每次 `.env` 持久化同时生成全部秘密（含口令）的 age 兼容加密快照
`secrets/secrets.age`，密文可安全提交 git、随备份或异地拷贝；主机或 `.env` 丢失后凭主密钥 + 快照
导入重建，主密钥与 `.env` 同时丢失则只能走 ADR-0040 重置路径。快照同时覆盖**文件型秘密**
（签名证书、keystore 等，[ADR-0042](../adr/0042-secret-files-signing-assets.md)）：明文存
`secrets/files/<用途名>/`（chmod 600，永不入库/落 data），与文本秘密整体打 tar 后 age 加密，
恢复时成套还原；每个文件登记用途、属主与有效期并在过期前告警；CI 构建期凭据注入临时取用，
禁止打进镜像或明文入库。
...
- 秘密按作用域（scope，默认对齐工程）分权：员工以本人账号在线取用仅被授权作用域的秘密，全部
  存取写审计且不含秘密值；admin 可按作用域生成独立口令加密的离线分发包（发放写审计），离线包
  不可远程撤销，人员变动以收回授权 + 轮换作用域秘密处置。决策见
  [ADR-0043](../adr/0043-secret-scopes-acl-audit.md)。
- 密钥体系为多接收者 age 密钥对（[ADR-0044](../adr/0044-recipient-key-hierarchy.md)）：系统自身
  秘密仅 admin 私钥可解；工程/签名秘密按接收者集合（admin + 授权 CI/员工）加密；提交秘密仅需
  接收者公钥，提交者不见明文；交给 CI/CD 的秘密只在构建环境内解密注入。一期先以 admin 单独
  显示/导出承载使用需求。
