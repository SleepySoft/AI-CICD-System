...
- 描述: 组件以插件化注册（config/tools.d/ 一文件一组件，用户目录同名覆盖）；每个组件可标记自启（supervisor 启动时自动拉起停止的容器）与关键组件（critical，关自启/停止需强确认）；登录用户可查看组件日志与运行详情（镜像/端口/运行时长/健康/重启次数）。
- 验收: 新增组件 = 丢一个 yaml 到 tools.d/ 即出现在首页；标记自启的组件在 supervisor 重启后自动拉起；critical 组件操作有确认提示；日志与详情接口可用。
