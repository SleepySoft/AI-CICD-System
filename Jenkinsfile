...
pipeline {
  ...
  stages {
    ...
    stage('沙箱可达性测试') {
      ...
      steps {
        // 同一 dockerd 上与部署操作互斥；workdir 置于 workspace 内（docker.sock 挂载下临时目录须宿主可见）
        lock('docker-sandbox') {
          sh 'python -m chronicler sandbox --workdir .sandbox --junit sandbox-report.xml --timeout 900 --probe-timeout 600'
        }
      }
      post {
